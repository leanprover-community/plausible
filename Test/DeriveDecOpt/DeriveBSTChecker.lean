...
info: Try this checker:
  instance : DecOpt (Between lo_1 x_1 hi_1) where
    decOpt :=
      let rec aux_dec (initSize : Nat) (size : Nat) (lo_1 : Nat) (x_1 : Nat) (hi_1 : Nat) : Except Plausible.GenError Bool :=
        (match size with
        | Nat.zero =>
...
info: Try this checker:
  ...
  instance : DecOpt (BST lo_1 hi_1 t_1) where
    decOpt :=
      let rec aux_dec (initSize : Nat) (size : Nat) (lo_1 : Nat) (hi_1 : Nat) (t_1 : BinaryTree) : Except Plausible.GenError Bool :=
        (match size with
        | Nat.zero =>
          DecOpt.checkerBacktrack
            [fun _ =>
              match t_1 with
              | BinaryTree.Leaf => Except.ok Bool.true
              | _ => Except.ok Bool.false]
        | Nat.succ size' =>
          DecOpt.checkerBacktrack
            [fun _ =>
