...
info: Try this checker:
  instance : DecOpt (InList x_1 l_1) where
  decOpt :=
    let rec aux_dec (initSize : Nat) (size : Nat) (x_1 : Nat) (l_1 : List Nat) : Except Plausible.GenError Bool :=
      (match size with
      | Nat.zero =>
...
/--
info: Try this checker:
  instance : DecOpt (MinOk l_1 a_1) where
  decOpt :=
    let rec aux_dec (initSize : Nat) (size : Nat) (l_1 : List Nat) (a_1 : List Nat) : Except Plausible.GenError Bool :=
      (match size with
      | Nat.zero =>
        DecOpt.checkerBacktrack
          [fun _ =>
...
/--
info: Try this checker:
  instance : DecOpt (MinEx n_1 l_1 a_1) where
  decOpt :=
    let rec aux_dec (initSize : Nat) (size : Nat) (n_1 : Nat) (l_1 : List Nat) (a_1 : List Nat) :
