...
info: Try this enumerator: instance : EnumSizedSuchThat Nat (fun x_1 => LessThanEq a_1 x_1) where
  enumSizedST :=
    let rec aux_enum (initSize : Nat) (size : Nat) (a_1 : Nat) : ExceptT Plausible.GenError Enumerator Nat :=
      (match size with
...
info: Try this enumerator: instance : EnumSizedSuchThat Nat (fun y_1 => LessThanEq x_1 y_1) where
  enumSizedST :=
    let rec aux_enum (initSize : Nat) (size : Nat) (x_1 : Nat) : ExceptT Plausible.GenError Enumerator Nat :=
      (match size with
...
info: Try this checker:
  instance : DecOpt (LessThanEq n_1 m_1) where
  decOpt :=
...
/--
info: Try this checker:
  instance : DecOpt (NatChain a_1 b_1) where
  decOpt :=
    let rec aux_dec (initSize : Nat) (size : Nat) (a_1 : Nat) (b_1 : Nat) : Except Plausible.GenError Bool :=
      (match size with
      | Nat.zero =>
        DecOpt.checkerBacktrack
          [fun _ =>
